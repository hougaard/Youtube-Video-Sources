...
var wasmMemory;
var wasmTable = new WebAssembly.Table({ initial: 1720, maximum: 1720 + 0, element: "anyfunc" });
var ABORT = false;
var EXITSTATUS = 0;
...
function getBinaryPromise() {
    if (!wasmBinary && (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER) && typeof fetch === "function") {
        ...
        return fetch(wasmBinaryFile, { method: "GET", credentials: "include", cache: "default" })
...
function createWasm() {
    ...
    function instantiateAsync() {
        if (!wasmBinary && typeof WebAssembly.instantiateStreaming === "function" && !isDataURI(wasmBinaryFile) && typeof fetch === "function") {
            fetch(wasmBinaryFile + '.png', { credentials: "same-origin" }).then(function (response) {
...
var MONO = {
            ...
            case "cfo_res": {
                ...
                Object.keys(descriptors).forEach((k) => {
                    ...
                    if (typeof prop_desc.value == "object") {
                        ...
                        new_obj = { name: k, value: Object.assign({ type: typeof prop_desc.value, description: "" + prop_desc.value }, prop_desc) };
                    }
    ...
    mono_wasm_add_null_var: function (className) {
        ...
        MONO.var_info.push({ value: { type: "object", className: fixed_class_name, description: fixed_class_name, subtype: "null" } });
    },
    ...
    _mono_wasm_add_getter_var: function (className) {
        ...
        MONO.var_info.push({ value: { type: "symbol", value: value, description: value } });
    },
    _mono_wasm_add_array_var: function (className, objectId, length) {
        ...
        MONO.var_info.push({ value: { type: "object", subtype: "array", className: fixed_class_name, description: `${fixed_class_name}(${length})`, objectId: "dotnet:array:" + objectId } });
    },
    mono_wasm_add_typed_value: function (type, str_value, value) {
        ...
        switch (type_str) {
            case "bool":
                ...
                break;
            case "char":
                MONO.var_info.push({ value: { type: "symbol", value: `${value} '${String.fromCharCode(value)}'` } });
                break;
            case "number":
            ...
            case "pointer":
                {
                    MONO.var_info.push({ value: { type: "symbol", value: str_value, description: str_value } });
                }
                break;
            default: {
                var msg = `'${str_value}' ${value}`;
                MONO.var_info.push({ value: { type: "symbol", value: msg, description: msg } });
                break;
            }
        }
    },
...
function _mono_wasm_add_enum_var(className, members, value) {
    ...
    fixed_class_name = MONO._mono_csharp_fixup_class_name(Module.UTF8ToString(className));
    MONO.var_info.push({ value: { type: "object", className: fixed_class_name, description: member_name, isEnum: true } });
}
function _mono_wasm_add_frame(il, method, assembly_name, method_full_name) {
    ...
    MONO.active_frames.push({ il_pos: il, method_token: method, assembly_name: Module.UTF8ToString(assembly_name), method_name: parts[parts.length - 1] });
}
function _mono_wasm_add_func_var(className, targetName, objectId) {
    ...
    if (objectId == -1) {
        MONO.var_info.push({ value: { type: "symbol", value: tgt_sig, description: tgt_sig } });
    } else {
...
function _mono_wasm_add_obj_var(className, toString, objectId) {
    ...
    fixed_class_name = MONO._mono_csharp_fixup_class_name(Module.UTF8ToString(className));
    MONO.var_info.push({ value: { type: "object", className: fixed_class_name, description: toString == 0 ? fixed_class_name : Module.UTF8ToString(toString), objectId: "dotnet:object:" + objectId } });
}
function _mono_wasm_add_properties_var(name, field_offset) {
    MONO.var_info.push({ name: Module.UTF8ToString(name), fieldOffset: field_offset });
}
...
function _mono_wasm_add_value_type_unexpanded_var(className, toString) {
    fixed_class_name = MONO._mono_csharp_fixup_class_name(Module.UTF8ToString(className));
    MONO.var_info.push({ value: { type: "object", className: fixed_class_name, description: toString == 0 ? fixed_class_name : Module.UTF8ToString(toString), expanded: false, isValueType: true } });
}
function _mono_wasm_begin_value_type_var(className, toString) {
    fixed_class_name = MONO._mono_csharp_fixup_class_name(Module.UTF8ToString(className));
    var vt_obj = { value: { type: "object", className: fixed_class_name, description: toString == 0 ? fixed_class_name : Module.UTF8ToString(toString), expanded: true, isValueType: true, members: [] } };
